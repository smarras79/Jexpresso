...
function write_vtk(SD::NSD_3D, mesh::St_mesh, q::Array, mp, 
    ...
    #=new_size = npoin
    iter = 1=#
    #=if ("periodicx" in mesh.bdy_face_type || "periodicz" in mesh.bdy_face_type || "periodicy" in mesh.bdy_face_type)
        new_size = size(mesh.x,1)
        diff = new_size-npoin
        q_new = zeros(new_size*nvar)
        q_exact1 = zeros(new_size,nvar+1)
                ...
                for j=1:mesh.ngl
                    for k=1:mesh.ngl
                        ...
                        if (ip == ip1)
                            ...
                            if (inputs[:ltwo_stream_radiation])
                                flux_lw_new[ip1] = mp.flux_lw[ip]
                                flux_sw_new[ip1] = mp.flux_sw[ip]
                            end 
                        else
                            for ieq = 1:nvars
                                ivar = new_size*(ieq-1)
