...
# PNG 1D
#
function write_output(SD::NSD_1D, q::Array, t, iout, mesh::St_mesh, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::PNG; nvar=1, qout=zeros(1,nvar), case="")
    #OK
...
function write_output(SD::NSD_1D, sol::ODESolution, mesh::St_mesh, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::PNG; nvar=1, qout=zeros(1,nvar), case="")
    #
...
# PNG 2D
#
function write_output(SD::NSD_2D, u::Array, t, iout, mesh::St_mesh, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::PNG; nvar=1, qout=zeros(1,nvar), case="")
    #
...
function write_output(SD::NSD_2D, sol::ODESolution, mesh::St_mesh, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::PNG; nvar=1, qout=zeros(1,nvar), case="")
    #
    # 2D PNG of q(t) from dq/dt = RHS
    #  
    if inputs[:lplot_surf3d]
...
function write_output(SD::NSD_2D, sol::SciMLBase.LinearSolution, mesh::St_mesh, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::PNG; nvar=1, qout=zeros(1,nvar), case="")
    #
...
# VTK 2D/3D
#
function write_output(SD, sol::ODESolution, mesh::St_mesh, mp, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::VTK; nvar=1, qout=zeros(1,nvar), case="")
 
    for iout = 1:size(sol.t[:],1)
        if (inputs[:backend] == CPU())
            title = @sprintf "final solution at t=%6.4f" sol.t[iout]
            write_vtk(SD, mesh, sol.u[iout][:], mp, sol.t[iout], title, OUTPUT_DIR, inputs, varnames; iout=iout, nvar=nvar, qout=qout, case=case)
        else
            ...
            KernelAbstractions.copyto!(CPU(),u_exact,qout)
            convert_mesh_arrays_to_cpu!(SD, mesh, inputs)
            title = @sprintf "final solution at t=%6.4f" sol.t[iout]
            write_vtk(SD, mesh, u, mp, sol.t[iout], title, OUTPUT_DIR, inputs, varnames; iout=iout, nvar=nvar, qout=u_exact, case=case)
        end
    end
...
function write_output(SD, sol::SciMLBase.LinearSolution, mesh::St_mesh, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::VTK; nvar=1, qout=zeros(1,nvar), case="")
    #
    # 2D VTK of x from Ax=b
    #    
    if (inputs[:backend] == CPU())
        title = @sprintf "Solution"
        write_vtk(SD, mesh, sol.u, "1", 0, title, OUTPUT_DIR, inputs, varnames; iout=1, nvar=nvar, qout=qout, case="")
    else
        ...
        convert_mesh_arrays_to_cpu!(SD, mesh, inputs)        
        title = @sprintf "Solution"
        write_vtk(SD, mesh, u, "1", title, OUTPUT_DIR, inputs, varnames; iout=1, nvar=nvar, qout=u_exact, case=case)
    end
...
function write_output(SD, u_sol, t, iout, mesh::St_mesh, mp, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::VTK; nvar=1, qout=zeros(1,nvar), case="")
    
    comm = MPI.COMM_WORLD
    rank = MPI.Comm_rank(comm)
    title = @sprintf "final solution at t=%6.4f" iout
    if (inputs[:backend] == CPU())
        write_vtk(SD, mesh, u_sol, mp, t, title, OUTPUT_DIR, inputs, varnames; iout=iout, nvar=nvar, qout=qout, case=case)        
    else
        #VERIFY THIS on GPU
        u = KernelAbstractions.allocate(CPU(),TFloat,mesh.npoin*(nvar+1))
        ...
        KernelAbstractions.copyto!(CPU(),u_exact,qout)
        convert_mesh_arrays_to_cpu!(SD, mesh, inputs)
        write_vtk(SD, mesh, u, mp, t, title, OUTPUT_DIR, inputs, varnames; iout=iout, nvar=nvar, qout=u_exact, case=case)
    end
...
# HDF5 writer/reader
#------------
function write_output(SD, u::AbstractArray, t, iout, mesh::St_mesh, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::HDF5; nvar=1, qout=zeros(1,nvar), case="")
    ...
    if (inputs[:backend] == CPU())
        ...
        write_hdf5(SD, mesh, u, qout, title, OUTPUT_DIR, inputs, varnames; iout=iout, nvar=nvar, case=case)
    else
...
function write_output(SD, sol::ODESolution, mesh::St_mesh, OUTPUT_DIR::String, inputs::Dict, varnames, outformat::HDF5; nvar=1, qout=zeros(1,nvar), case="")
    ...
    if (inputs[:backend] == CPU())
        write_hdf5(SD, mesh, sol.u[iout][:], qout, title, OUTPUT_DIR, inputs, varnames; iout=iout, nvar=nvar, case=case)
    else
        u_gpu = KernelAbstractions.allocate(CPU(),TFloat,mesh.npoin*nvar)
...
function write_vtk(SD::NSD_2D, mesh::St_mesh, q::Array, mp, t, title::String, OUTPUT_DIR::String, inputs::Dict, varnames; iout=1, nvar=1, qout=zeros(1,nvar), case="")
	...
	q_new = zeros(new_size*nvar)
        ...
        q_exact1[1:mesh.npoin,:] .= qout[1:mesh.npoin,:] 
        for ieq = 1:nvars
	    ivar = new_size*(ieq-1)
    ...
    if (inputs[:CL] == CL())
        ...
        if (inputs[:SOL_VARS_TYPE] == TOTAL())
            ...
            if (case == "rtb" || case == "mountain") && nvars >= 4
                ...
                if (size(qout, 1) == npoin)

                    if inputs[:loutput_pert] == true
                        
                        #ρ'
                        qout[1:npoin] .= q[1:npoin] .- qout[1:npoin,1]
                        
                        #θ' = (ρθ - ρθref)/ρ = ρθ/ρ - ρrefθref/ρref
                        ivar = 4
                        idx = (ivar - 1)*npoin
                        qout[idx+1:4*npoin] .= q[idx+1:4*npoin]./q[1:npoin] .- qout[1:npoin,4]./qout[1:npoin,1]
                    else
            ...
            for ivar = 2:nvars
                ...
                idx = (ivar - 1)*npoin
                qout[idx+1:ivar*npoin] .= (q[idx+1:ivar*npoin] .+ qout[1:npoin,ivar])./(qout[1:npoin] .+ qout[1:npoin,1]) .- qout[1:npoin,ivar]./qout[1:npoin,1]
                ...
                if (case == "rtb" || case == "mountain") && nvars >= 4
                    ...
                    if (size(qout, 1) === npoin)
                        ...
                        ivar = 4
                        idx = (ivar - 1)*npoin
                        qout[idx+1:4*npoin] .= (q[idx+1:4*npoin] .+ qout[1:npoin,4])./(qout[1:npoin] .+ qout[1:npoin,1]) .- qout[1:npoin,4]./qout[1:npoin,1]
                    end
                end
            end
        end
    ...
    elseif (inputs[:CL] == NCL())
        ...
        for ivar = 1:nvars
            ...
            if case === "rtb" && nvars >= 4
                ...
                if (inputs[:loutput_pert] == true && size(qout, 1) === npoin)
                    outvars = ("dρ", "u", "v", "dθ")
                    #ρ'
                    qout[1:npoin] .= (q[1:npoin] .- qout[1:npoin,1])
                    ...
                    ivar = 4
                    idx = (ivar - 1)*npoin
                    qout[idx+1:4*npoin] .= (q[idx+1:4*npoin] .- qout[1:npoin,4])
...
function write_vtk(SD::NSD_3D, mesh::St_mesh, q::Array, mp, t, title::String, OUTPUT_DIR::String, inputs::Dict, varnames; iout=1, nvar=1, qout=zeros(1,nvar), case="")
    outvars = varnames
    nvars = length(outvars)
    npoin = mesh.npoin
    xx = zeros(size(mesh.x,1))
            ...
            if (mesh.bdy_face_type[iface] == "periodic1")
                ...
                for k=1:mesh.ngl
                    for l=1:mesh.ngl
                        ...
                        DX = xface + (mesh.ngl)*dx
                        x_spare .= mesh.x .- DX .> 0
                        for i=1:mesh.ngl
                            for j=1:mesh.ngl
                                for kk=1:mesh.ngl
                                    ip1 = mesh.connijk[e,i,j,kk]
                                    if (x_spare[ip1])#xface + (mesh.ngl)*dx)
                                        unwind=1
                                    end
                                    if (ip1 == ip)
                                        ll=i
                        ...
                        if (k == 1 || k == mesh.ngl || l == 1 || l == mesh.ngl)
                            connijk_spare .= mesh.connijk .- npoin
                            for ee=1:mesh.nelem
                                for i=1:mesh.ngl
                                    for j=1:mesh.ngl
                                        for kk=1:mesh.ngl
                                            ip1 = mesh.connijk[ee,i,j,kk]
                                            if (connijk_spare[ee,i,j,kk] > 0 && mesh.y[ip1] == mesh.y[ip] && mesh.z[ip1] == mesh.z[ip])
                                                ip_rep = ip1
                                                rep = 1
                                            end
                                        end
                            ...
                            for ee=1:mesh.nelem
                                for i=1:mesh.ngl
                                    for j=1:mesh.ngl
                                        for kk=1:mesh.ngl
                                            ip1 = mesh.connijk[ee,i,j,kk]
                                            if (connijk_spare[ee,i,j,kk] > 0 && mesh.x[ip1] == mesh.x[ip] && mesh.y[ip1] == mesh.y[ip])
                                                ip_rep = ip1
                                                rep = 1
                                            end
                                        end
                            ...
                            for ee=1:mesh.nelem
                                for i=1:mesh.ngl
                                    for j=1:mesh.ngl
                                        for kk=1:mesh.ngl
                                            ip1 = mesh.connijk[ee,i,j,kk]
                                            if (connijk_spare[ee,i,j,kk] > 0 && mesh.x[ip1] == mesh.x[ip] && mesh.z[ip1] == mesh.z[ip])
                                                ip_rep = ip1
                                                rep = 1
                                            end
                                        end
            ...
            if (case == "rtb" || case == "mountain" || case == "bomex") && nvars >= 4
                ...
                if (size(qout, 1) == npoin)
                    ...
                    if inputs[:loutput_pert] == true
                        ...
                        ivar = 5
                        idx = (ivar - 1)*npoin
                        qout[idx+1:5*npoin] .= q[idx+1:5*npoin]./q[1:npoin] .- qout[1:npoin,5]./qout[1:npoin,1]
                    else
                        ivar = 5
                        idx = (ivar - 1)*mesh.npoin
            ...
            for ivar = 2:nvars
                ...
                if (case == "rtb" || case == "mountain") && nvars >= 4
                    ...
                    if (size(qout, 1) === npoin)
                        ...
                        ivar = 5
                        idx = (ivar - 1)*npoin
                        qout[idx+1:5*npoin] .= (q[idx+1:5*npoin] .+ qout[1:npoin,5])./(qout[1:npoin] .+ qout[1:npoin,1]) .- qout[1:npoin,5]./qout[1:npoin,1]
                    end
                end
            end
...
function write_vtk_ref(SD::NSD_2D, mesh::St_mesh, q::Array, file_name::String, OUTPUT_DIR::String; iout=1, nvar=1, qout=zeros(1,nvar), case="", outvarsref=tuple(("" for _ in 1:nvar)))
...
function write_vtk_ref(SD::NSD_3D, mesh::St_mesh, q::Array, file_name::String, OUTPUT_DIR::String; iout=1, nvar=1, qout=zeros(1,nvar), case="", outvarsref=tuple(("" for _ in 1:nvar)))
