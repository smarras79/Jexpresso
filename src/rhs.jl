...
function _build_rhs!(RHS, u, params, time)
    ...
    draw_results(params, Coef, time, result)
...
function inviscid_rhs_el!(u, params, connijk, qe, x, y, lsource, SD::NSD_2D)
    ...
    derivative_y1, derivative_x1 = compute_derivative(params, connijk, x, y)

    for iel = 1:params.mesh.nelem

        for l = 1:params.mesh.ngl, k=1:params.mesh.ngl
            ip = connijk[iel,k,l]

            user_primitives!(@view(params.uaux[ip,:]), @view(qe[ip,:]), @view(params.uprimitive[k,l,:]), params.SOL_VARS_TYPE)
            
        #=  user_flux!(@view(params.F[k,l,:]), @view(params.G[k,l,:]), SD,
                       @view(params.uaux[ip,:]),
                       @view(qe[ip,:]),         #pref
                       params.mesh,
                       params.CL, params.SOL_VARS_TYPE;
            ...
            user_flux!(@view(params.F[k,l,:]), @view(params.G[k,l,:]), SD,
                       @view(params.uaux[ip,:]),derivative_y1[ip],derivative_x1[ip],
                      @view(qe[ip,:]),         #pref
                      params.mesh,
                      params.CL, params.SOL_VARS_TYPE;
                      neqs=params.neqs, ip=ip)
...
function compute_derivative(params, connijk, x, y)
    ...
    # 1. average method
    derivative_y1 = zeros(TFloat, maximum(connijk))
    derivative_x1 = zeros(TFloat, maximum(connijk))
    count = zeros(TFloat,maximum(connijk))
        ...
        for l = 1:ngl,k = 1:ngl
            ...
            for j = 1:params.mesh.ngl
                for i = 1:params.mesh.ngl# which basis
                    ...
                    derivative_y1[ip] = derivative_y1[ip] .+ Coef[J].*dψIJ_dy
                    derivative_x1[ip] = derivative_x1[ip] .+ Coef[J].*dψIJ_dx
    ...
    derivative_y1 = derivative_y1./count
    derivative_x1 = derivative_x1./count

    return derivative_y1, derivative_x1
