function periodicity_restructure!(mesh,x,y,z,xmax,xmin,ymax,ymin,zmax,zmin,poin_in_bdy_face,poin_in_bdy_edge,ngl,ngr,nelem,npoin,nsd,bdy_edge_type,
    ...
    x_spare = zeros(mesh.npoin)
    y_spare = zeros(mesh.npoin) 
    z_spare = zeros(mesh.npoin) 
    if (mesh.nsd == 2)
                ...
                while (i1 <= size(per1_points,1) && found == false)
                    ...
                    if (determine_colinearity(vec, nor1))
                        ...
                        if !(ip_kill in mesh.connijk)
                            ...
                            @view(mesh.x[ip_kill:mesh.npoin-1]) .= @view(x_spare[ip_kill+1:mesh.npoin])
                            @view(mesh.y[ip_kill:mesh.npoin-1]) .= @view(y_spare[ip_kill+1:mesh.npoin])
                            @view(mesh.z[ip_kill:mesh.npoin-1]) .= @view(z_spare[ip_kill+1:mesh.npoin])
                            x_spare .= mesh.x
                            y_spare .= mesh.y
                            z_spare .= mesh.z
                            mesh.npoin = mesh.npoin-1
