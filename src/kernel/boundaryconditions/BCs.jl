      ...
      for iface=1:size(mesh.xmin_faces,2)
          for k=1:mesh.ngl
              for i=1:mesh.ngl
                  ...
                  dqdx_st[:,1] .= 0.5*(gradq[1,ip,:] .+ flux_q[k,iface,1,:] .- nx[k,iface]*mu.*(exact[k,iface,:].-qp.qn[ip,1:nvars]))
                  dqdx_st[:,2] .= 0.5*(gradq[2,ip,:] .+ flux_q[k,iface,2,:] .- ny[k,iface]*mu.*(exact[k,iface,:].-qp.qn[ip,1:nvars]))
                  q_st[:] .= 0.5*(qp.qn[ip,1:nvars] + exact[k,iface,:]) 
                  ...
                  rhs[1,k,iel,1:nvars] .-= ω[k]*metrics.Jef[k,iface]*(nx[k,iface])*dψ[i,k].*(qp.qn[ip,1:nvars] .- q_st[1:nvars])
      ...
      for iface=1:size(mesh.xmax_faces,2)
          for k=1:mesh.ngl
              for i=1:mesh.ngl
                  ...
                  dqdx_st[:,1] .= 0.5*(gradq[1,ip,:] .+ flux_q[k,iface+disp,1,:] - nx[k,iface+disp]*mu.*(exact[k,iface+disp,:].-qp.qn[ip,1:nvars]))
                  dqdx_st[:,2] .= 0.5*(gradq[2,ip,:] .+ flux_q[k,iface+disp,2,:] - ny[k,iface+disp]*mu.*(exact[k,iface+disp,:].-qp.qn[ip,1:nvars]))
                  q_st[:] = 0.5*(qp.qn[ip,1:nvars] + exact[k,iface+disp,:])
                  ...
                  rhs[mesh.ngl,k,iel,1:nvars] .-= ω[k]*metrics.Jef[k,iface+size(mesh.xmin_faces,2)]*(nx[k,iface+disp])*dψ[i,k].*(qp.qn[ip,1:nvars] .- q_st[1:nvars])
                  #if (qp.qn[ip,1] < -0.01)
          ...
          for k=1:mesh.ngl
              for i=1:mesh.ngl
                ...
                #  @info "2",rhs[k,1,iel,1:nvars], mesh.x[ip],mesh.y[ip]
                  rhs[k,1,iel,1:nvars] .-= ω[k]*metrics.Jef[k,iface+disp]*(ny[k,iface+disp])*dψ[i,k].*(qp.qn[ip,1:nvars] .- q_st[1:nvars])
                #  @info "3",rhs[k,1,iel,1:nvars], mesh.x[ip],mesh.y[ip]
                   #if (qp.qn[ip,1] < -0.01)
              #   @info "ymin", rhs[k,1,iel,3],rhs[k,1,iel,2]
#     @info "ymin",mesh.x[mesh.connijk[k,1,iel]],mesh.y[mesh.connijk[k,1,iel]],rhs[k,1,iel,3],ψ[i,k]*(ny[k,iface+disp]*dqdx_st[3,2]),(ny[k,iface+disp])*dψ[i,k]*(qp.qn[ip,3] - q_st[3]),q_st[3]
                  #end 
              end
          end
      end
      ...
      for iface=1:size(mesh.ymax_faces,2)
          for k=1:mesh.ngl
              for i=1:mesh.ngl
                   ...
                   rhs[k,mesh.ngl,iel,1:nvars] .-= ω[k]*metrics.Jef[k,iface+disp]*(ny[k,iface+disp])*dψ[i,k].*(qp.qn[ip,1:nvars] .- q_st[1:nvars])
                  #if (qp.qn[ip,1] < -0.01)
...
function build_custom_bcs!(t,mesh,q,gradq,::NSD_2D,::DefaultBC,exact_q,flux_q,nx,ny,nvars)
    for k=1:size(mesh.xmin_faces,2)
          for i=1:mesh.ngl
              ...
              exact_q[i,k,1] = -q.qn[ip,1]
              exact_q[i,k,2] = q.qn[ip,2] - 2*unl*nx[i,k]
              exact_q[i,k,3] = q.qn[ip,3] - 2*unl*ny[i,k]
              for var=1:nvars
      ...
      for k=1:size(mesh.xmax_faces,2)
          for i=1:mesh.ngl
              ...
              exact_q[i,kf,1] = -q.qn[ip,1]
              exact_q[i,kf,2] = q.qn[ip,2] - 2*unl*nx[i,kf]
              exact_q[i,kf,3] = q.qn[ip,3] - 2*unl*ny[i,kf]
              for var=1:nvars
