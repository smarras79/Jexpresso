...
function build_custom_bcs!(t,mesh,q,gradq,rhs,::NSD_2D,nvars,metrics,ω,dirichlet!,neumann,L,inputs)
    ...
    for iedge = 1:size(mesh.bdy_edge_in_elem,1)
        ...
        comp = mesh.bdy_edge_comp[iedge]
        for k=1:mesh.ngl
            if (mesh.bdy_edge_type[iedge] != "periodic1" && mesh.bdy_edge_type[iedge] !="periodic2") #&& mesh.bdy_edge_type[iedge] !="Laguerre")
                tag = mesh.bdy_edge_type[iedge]
                ip = mesh.poin_in_bdy_edge[iedge,k]
                mm=1
                ll=1
                ...
                if (inputs[:luser_bc])
                    ...
                    ipp=1 #ip                    
                    qbdy = dirichlet!(q[ip,:],gradq[:,ipp,:],x,y,t,mesh,metrics.nx[k,iedge],metrics.ny[k,iedge],tag,qbdy,inputs)
