       ...
       for iface=1:size(mesh.ymin_faces,2)
          for k=1:mesh.ngl
              for i=1:mesh.ngl
                  ...
                  dqdx_st[:,2] = 0.5*(gradq[2,ip,:] .+ flux_q[ip,2,:] - ny[ip]*mu.*(exact[ip,:].-qp.qn[ip,1:nvars]))
                  q_st[:] = 0.5*(qp.qn[ip,1:nvars] + exact[ip,:])
                  rhs[k,1,iel,1:nvars] .-= ω[k]*metrics.Jef[k,iface+disp]*ψ[i,k]*(nx[ip]*dqdx_st[1:nvars,1] .+ ny[ip]*dqdx_st[1:nvars,2])
      ...
      for iface=1:size(mesh.ymax_faces,2)
          for k=1:mesh.ngl
              for i=1:mesh.ngl
                   ...
                   dqdx_st[:,2] = 0.5*(gradq[2,ip,:] .+ flux_q[ip,2,:] - ny[ip]*mu.*(exact[ip,:].-qp.qn[ip,1:nvars]))
                   q_st[:] = 0.5*(qp.qn[ip,1:nvars] + exact[ip,:])
                   rhs[k,mesh.ngl,iel,1:nvars] .-= ω[k]*metrics.Jef[k,iface+disp]*ψ[i,k].*(nx[ip]*dqdx_st[1:nvars,1] .+ ny[ip]*dqdx_st[1:nvars,2])
