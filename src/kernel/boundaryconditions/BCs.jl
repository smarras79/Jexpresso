...
function apply_periodicity!(rhs,qp,mesh,inputs, SD::NSD_2D,QT,metrics,ψ,dψ, ω,t,BCT,neqns)
    #Periodic boundaries
...
function apply_boundary_conditions!(rhs,qp,mesh,inputs, SD::NSD_2D,QT,metrics,ψ,dψ, ω,t,BCT,neqns)
    ...
    dqdx_st = zeros(neqns,2)
    q_st = zeros(neqns,1)
    gradq = zeros(2,mesh.npoin,neqns)
    flux_q = zeros(mesh.ngl,nface,2,neqns)
    exact = zeros(mesh.ngl,nface,neqns)
    penalty =160.0#50000
    nx = zeros(mesh.ngl,nface)
    ny = zeros(mesh.ngl,nface)
    if (calc_grad)
        gradq = build_gradient(SD, QT::Inexact, qp, ψ, dψ, ω, mesh, metrics,gradq,neqns)
        build_custom_bcs!(t,mesh,qp,gradq,SD,BCT,exact,flux_q,nx,ny,neqns)
    end
        ...
        for iface=1:size(mesh.xmin_faces,2)
            for k=1:mesh.ngl
                for i=1:mesh.ngl
                    ...
                    #@info "1",rhs[1,k,iel,1:neqns], mesh.x[ip],mesh.y[ip]
                    dqdx_st[:,1] .= 0.5*(gradq[1,ip,:] .+ flux_q[k,iface,1,:] .- nx[k,iface]*mu.*(exact[k,iface,:].-qp[ip,1:neqns]))
                    dqdx_st[:,2] .= 0.5*(gradq[2,ip,:] .+ flux_q[k,iface,2,:] .- ny[k,iface]*mu.*(exact[k,iface,:].-qp[ip,1:neqns]))
                    q_st[:] .= 0.5*(qp[ip,1:neqns] + exact[k,iface,:]) 
                    rhs[1,k,iel,1:neqns] .-= ω[k]*metrics.Jef[k,iface]*ψ[i,k].*(nx[k,iface]*dqdx_st[1:neqns,1]) #.+ ny[ip]*dqdx_st[1:neqns,2])
                    #@info "2",rhs[1,k,iel,1:neqns],mesh.x[ip],mesh.y[ip]
                    rhs[1,k,iel,1:neqns] .-= ω[k]*metrics.Jef[k,iface]*(nx[k,iface])*dψ[i,k].*(qp[ip,1:neqns] .- q_st[1:neqns])
                    #@info "3",rhs[1,k,iel,1:neqns],mesh.x[ip],mesh.y[ip]
                    #end
                end
            end
        end
        ...
        for iface=1:size(mesh.xmax_faces,2)
            for k=1:mesh.ngl
                for i=1:mesh.ngl
                    ...
                    dqdx_st[:,1] .= 0.5*(gradq[1,ip,:] .+ flux_q[k,iface+disp,1,:] - nx[k,iface+disp]*mu.*(exact[k,iface+disp,:].-qp[ip,1:neqns]))
                    dqdx_st[:,2] .= 0.5*(gradq[2,ip,:] .+ flux_q[k,iface+disp,2,:] - ny[k,iface+disp]*mu.*(exact[k,iface+disp,:].-qp[ip,1:neqns]))
                    q_st[:] = 0.5*(qp[ip,1:neqns] + exact[k,iface+disp,:])
                    rhs[mesh.ngl,k,iel,1:neqns] .-= ω[k]*metrics.Jef[k,iface+size(mesh.xmin_faces,2)]*ψ[i,k].*(nx[k,iface+disp]*dqdx_st[1:neqns,1]) #.+ ny[ip]*dqdx_st[1:neqns,2])
                    rhs[mesh.ngl,k,iel,1:neqns] .-= ω[k]*metrics.Jef[k,iface+size(mesh.xmin_faces,2)]*(nx[k,iface+disp])*dψ[i,k].*(qp[ip,1:neqns] .- q_st[1:neqns])
                    #if (qp[ip,1] < -0.01)
        ...
        for iface=1:size(mesh.ymin_faces,2)
            for k=1:mesh.ngl
                for i=1:mesh.ngl
                    ...
                    dqdx_st[:,2] .= 0.5*(gradq[2,ip,:] .+ flux_q[k,iface+disp,2,:] - ny[k,iface+disp]*mu.*(exact[k,iface+disp,:].-qp[ip,1:neqns]))
                    q_st[:] = 0.5*(qp[ip,1:neqns] + exact[k,iface+disp,:])
                    #  @info "1",rhs[k,1,iel,1:neqns], mesh.x[ip],mesh.y[ip]
                    rhs[k,1,iel,1:neqns] .-= ω[k]*metrics.Jef[k,iface+disp]*ψ[i,k]*(ny[k,iface+disp]*dqdx_st[1:neqns,2])
                    #  @info "2",rhs[k,1,iel,1:neqns], mesh.x[ip],mesh.y[ip]
                    rhs[k,1,iel,1:neqns] .-= ω[k]*metrics.Jef[k,iface+disp]*(ny[k,iface+disp])*dψ[i,k].*(qp[ip,1:neqns] .- q_st[1:neqns])
                    #  @info "3",rhs[k,1,iel,1:neqns], mesh.x[ip],mesh.y[ip]
                    #if (qp[ip,1] < -0.01)
    ...
    for iface=1:size(mesh.ymax_faces,2)
        for k=1:mesh.ngl
            for i=1:mesh.ngl
                ...
                dqdx_st[:,2] .= 0.5*(gradq[2,ip,:] .+ flux_q[k,iface+disp,2,:] - ny[k,iface+disp]*mu.*(exact[k,iface+disp,:].-qp[ip,1:neqns]))
                q_st[:] = 0.5*(qp[ip,1:neqns] + exact[k,iface+disp,:])
                rhs[k,mesh.ngl,iel,1:neqns] .-= ω[k]*metrics.Jef[k,iface+disp]*ψ[i,k].*(0.0*dqdx_st[1:neqns,1] .+ ny[k,iface+disp]*dqdx_st[1:neqns,2])
                rhs[k,mesh.ngl,iel,1:neqns] .-= ω[k]*metrics.Jef[k,iface+disp]*(ny[k,iface+disp])*dψ[i,k].*(qp[ip,1:neqns] .- q_st[1:neqns])
                #if (qp[ip,1] < -0.01)
...
function apply_periodicity!(qp,mesh,inputs,SD::NSD_3D,QT,metrics,ψ,dψ, ω,t,BCT,neqns)
    ...
    if (haskey(inputs, :xmin_bc) && inputs[:xmin_bc]=="periodic" || haskey(inputs, :xmax_bc) && inputs[:xmax_bc]=="periodic")
        ...
        for I in keys(mesh.xperiodicity)
            ip = mesh.xperiodicity[I]
            qp[I,1:neqns] .= 0.5*(qp[I,1:neqns] .+ qp[ip,1:neqns])
            qp[ip,1:neqns] .= qp[I,1:neqns]
        end
    end
    if (haskey(inputs, :ymin_bc) && inputs[:ymin_bc]=="periodic") || (haskey(inputs, :ymax_bc) && inputs[:ymax_bc]=="periodic")
        inputs[:ymax_bc] = "periodic"
        ...
        for I in keys(mesh.yperiodicity)
            ip = mesh.yperiodicity[I]
            qp[I,:] .= 0.5*(qp[I,1:neqns] .+ qp[ip,1:neqns])
            qp[ip,:] .= qp[I,:]
        end
    end
...
function apply_boundary_conditions!(rhs,qp,mesh,inputs, SD::NSD_3D,QT,metrics,ψ,dψ, ω,t,BCT,neqns)
    calc_grad = false
    for key in keys(inputs)
        if (inputs[key] == "dirichlet" || inputs[key] == "neumann" || inputs[key] == "dirichlet/neumann")
            calc_grad = true
    ...
    #  @info calc_grad
    dqdx_st = zeros(neqns,3)
    q_st = zeros(neqns,1)
    gradq = zeros(mesh.npoin,3,neqns)
    flux_q = zeros(mesh.npoin,3,neqns)
    exact = zeros(mesh.npoin,neqns)
    penalty = 0.0#50000
                ...
                for l=1:mesh.ngl
                    for i=1:mesh.ngl
                        for j=1:mesh.ngl
                            ...
                            dqdx_st[:,1] = 0.5*(gradq[1,ip,:] .+ flux_q[ip,1,:] - nx[ip]*mu.*(exact[ip,:].-qp[ip,1:neqns]))
                            dqdx_st[:,2] = 0.5*(gradq[2,ip,:] .+ flux_q[ip,2,:] - ny[ip]*mu.*(exact[ip,:].-qp[ip,1:neqns]))
                            dqdx_st[:,3] = 0.5*(gradq[3,ip,:] .+ flux_q[ip,3,:] - nz[ip]*mu.*(exact[ip,:].-qp[ip,1:neqns]))
                            q_st[:] = 0.5*(qp[ip,1:neqns] + exact[ip,:])
                            rhs[1,i,j,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*ψ[i,k]*ψ[j,l]*(nx[ip]*dqdx_st[1:neqns,1] .+ ny[ip]*dqdx_st[1:neqns,3] .+ nz[ip]*dqdx_st[1:neqns,3])
                            rhs[1,i,j,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*(nx[ip])*dψ[i,k]*dψ[j,l]*(qp[ip,1:neqns] - q_st[1:neqns])
                            #if (qp[ip,1] < -0.01)
                ...
                for l=1:mesh.ngl
                    for i=1:mesh.ngl
                        for j=1:mesh.ngl
                            ...
                            dqdx_st[:,3] = 0.5*(gradq[3,ip,:] .+ flux_q[ip,3,:] - nz[ip]*mu.*(exact[ip,:].-qp[ip,1:neqns]))
                            q_st[:] = 0.5*(qp[ip,1:neqns] + exact[ip,:])
                            rhs[mesh.ngl,i,j,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*ψ[i,k]*ψ[j,l]*(nx[ip]*dqdx_st[1:neqns,1] .+ ny[ip]*dqdx_st[1:neqns,3] .+ nz[ip]*dqdx_st[1:neqns,3])
                            rhs[mesh.ngl,i,j,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*(nx[ip])*dψ[i,k]*dψ[j,l]*(qp[ip,1:neqns] - q_st[1:neqns])      
                            #if (qp[ip,1] < -0.01)
                            # @info calc_grad,rhs[ip],ω[k],metrics.Jef[k,iface],(nx[ip]),dψ[i,k],(qp[ip,1] - q_st[1])
                            #end
                        end
                ...
                for l=1:mesh.ngl
                    for i=1:mesh.ngl
                        for j=1:mesh.ngl
                            ...
                            dqdx_st[:,3] = 0.5*(gradq[3,ip,:] .+ flux_q[ip,3,:] - nz[ip]*mu.*(exact[ip,:].-qp[ip,1:neqns]))
                            q_st[:] = 0.5*(qp[ip,1:neqns] + exact[ip,:])
                            rhs[i,1,j,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*ψ[i,k]*ψ[j,l]*(nx[ip]*dqdx_st[1:neqns,1] .+ ny[ip]*dqdx_st[1:neqns,3] .+ nz[ip]*dqdx_st[1:neqns,3])
                            rhs[i,1,j,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*(nx[ip])*dψ[i,k]*dψ[j,l]*(qp[ip,1:neqns] - q_st[1:neqns])
                            #if (qp[ip,1] < -0.01)
                            # @info calc_grad,rhs[ip],ω[k],metrics.Jef[k,iface],(nx[ip]),dψ[i,k],(qp[ip,1] - q_st[1])
                            #end
                        end
            ...
            for l=1:mesh.ngl
                for i=1:mesh.ngl
                    for j=1:mesh.ngl
                        ...
                        dqdx_st[:,3] = 0.5*(gradq[3,ip,:] .+ flux_q[ip,3,:] - nz[ip]*mu.*(exact[ip,:].-qp[ip,1:neqns]))
                        q_st[:] = 0.5*(qp[ip,1:neqns] + exact[ip,:])
                        rhs[i,mesh.ngl,j,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*ψ[i,k]*ψ[j,l]*(nx[ip]*dqdx_st[1:neqns,1] .+ ny[ip]*dqdx_st[1:neqns,3] .+ nz[ip]*dqdx_st[1:neqns,3])
                        rhs[i,mesh.ngl,j,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*(nx[ip])*dψ[i,k]*dψ[j,l]*(qp[ip,1:neqns] - q_st[1:neqns])
                        #if (qp[ip,1] < -0.01)
                        # @info calc_grad,rhs[ip],ω[k],metrics.Jef[k,iface],(nx[ip]),dψ[i,k],(qp[ip,1] - q_st[1])
                        #end
                    end
            ...
            for l=1:mesh.ngl
                for i=1:mesh.ngl
                    for j=1:mesh.ngl
                        ...
                        dqdx_st[:,3] = 0.5*(gradq[3,ip,:] .+ flux_q[ip,3,:] - nz[ip]*mu.*(exact[ip,:].-qp[ip,1:neqns]))
                        q_st[:] = 0.5*(qp[ip,1:neqns] + exact[ip,:])
                        rhs[i,j,1,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*ψ[i,k]*ψ[j,l]*(nx[ip]*dqdx_st[1:neqns,1] .+ ny[ip]*dqdx_st[1:neqns,3] .+ nz[ip]*dqdx_st[1:neqns,3])
                        rhs[i,j,1,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*(nx[ip])*dψ[i,k]*dψ[j,l]*(qp[ip,1:neqns] - q_st[1:neqns])
                        #if (qp[ip,1] < -0.01)
                        # @info calc_grad,rhs[ip],ω[k],metrics.Jef[k,iface],(nx[ip]),dψ[i,k],(qp[ip,1] - q_st[1])
                        #end
                    end
            ...
            for l=1:mesh.ngl
                for i=1:mesh.ngl
                    for j=1:mesh.ngl
                        ...
                        dqdx_st[:,3] = 0.5*(gradq[3,ip,:] .+ flux_q[ip,3,:] - nz[ip]*mu.*(exact[ip,:].-qp[ip,1:neqns]))
                        q_st[:] = 0.5*(qp[ip,1:neqns] + exact[ip,:])
                        rhs[i,j,mesh.ngl,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*ψ[i,k]*ψ[j,l]*(nx[ip]*dqdx_st[1:neqns,1] .+ ny[ip]*dqdx_st[1:neqns,3] .+ nz[ip]*dqdx_st[1:neqns,3])
                        rhs[i,j,mesh.ngl,iel,1:neqns] .-= ω[k]*ω[l]*metrics.Jef[k,l,iface]*(nx[ip])*dψ[i,k]*dψ[j,l]*(qp[ip,1:neqns] - q_st[1:neqns])
                        #if (qp[ip,1] < -0.01)
                        # @info calc_grad,rhs[ip],ω[k],metrics.Jef[k,iface],(nx[ip]),dψ[i,k],(qp[ip,1] - q_st[1])
                        #end
                    end
...
function build_custom_bcs!(t,mesh,q,gradq,::NSD_2D,::DefaultBC,exact_q,flux_q,nx,ny,neqns)
...
function build_custom_bcs!(t,mesh,q,gradq,::NSD_2D,::LinearClaw_KopNR,exact_q,flux_q,nx,ny,neqns)
    c  = 1.0
    x0 = y0 = -0.8
    kx = ky = sqrt(2)/2
    ω  = 0.2
...
function build_custom_bcs!(t,mesh,q,gradq,::NSD_2D,::LinearClaw_KopRefxmax,exact_q,flux_q,nx,ny,neqns)
    c  = 1.0
    x0 = y0 = -0.8
    kx = ky = sqrt(2.0)/2
    ω  = 0.2
...
function build_custom_bcs!(t,mesh,q,qe,gradq,::NSD_3D,::DefaultBC,exact_q,flux_q,nx,ny,nz,neqns)
    npoin = mesh.npoin
    for ip = 1:mesh.npoin
        x = mesh.x[ip]
        y = mesh.y[ip]
