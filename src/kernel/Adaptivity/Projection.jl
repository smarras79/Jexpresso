...
function mod_mesh_adaptive!(partitioned_model_coarse, ref_coarse_flags, omesh, mesh::St_mesh, inputs, nparts, distribute)
    ...
    # Resize as needed
    #
    # These three lines used to REBIND mesh.x/y/z to fresh arrays after npoin
    # changed. coords is one array, so the equivalent is one reallocation --
    # and it must stay three rows, which is what every consumer now indexes.
    mesh.coords = KernelAbstractions.zeros(backend, TFloat, 3, Int64(mesh.npoin))
    ...
    for ip = 1: mesh.npoin
        if mesh.gip2owner[ip] != rank+1
            # @info mesh.coords[1,ip], mesh.coords[2,ip], mesh.gip2owner[ip], rank+1
        end
    end
    ...
    if mesh.nsd == 2
        ...
        for iedge = 1:mesh.nedges #total nedges
            if isboundary_edge[iedge] == true
                # if rank == 1
                #     @info mesh.coords[1,mesh.poin_in_edge[iedge, 1]], mesh.coords[2,mesh.poin_in_edge[iedge, 1]]
                # end
                for igl = 1:mesh.ngl
        ...
        if ("Laguerre" in mesh.bdy_edge_type)
            ...
            x_new[1:mesh.npoin] .= @view(mesh.coords[1,:])
            y_new[1:mesh.npoin] .= @view(mesh.coords[2,:])
                    ...
                    for i=1:mesh.ngl
                        ...
                        for j=2:mesh.ngr
                            ...
                            if (i == mesh.ngl || i == 1)
                                ...
                                x_new[iter] = x_temp#mesh.coords[1,ip] + nor[1]*gr.ξ[j]*factorx
                                y_new[iter] = y_temp#mesh.coords[2,ip] + nor[2]*gr.ξ[j]*factory
                            ...
                            #@info nor[1],nor[2],x_new[iter],y_new[iter], mesh.coords[1,ip],mesh.coords[2,ip]
                        end
                    end
                    e_iter += 1
                end
            ...
            mesh.npoin = iter -1
            # Rebind, not element-write: mesh.npoin has just changed, so the
            # array is reallocated at the new size and the (possibly longer)
            # x_new/y_new are copied into its first mesh.npoin columns. Row 3
            # stays zero, as the z array it replaces was here.
            mesh.coords = KernelAbstractions.zeros(backend, TFloat, 3, Int64(mesh.npoin))
            @views mesh.coords[1,1:mesh.npoin] .= x_new[1:mesh.npoin]
            @views mesh.coords[2,1:mesh.npoin] .= y_new[1:mesh.npoin]
...
function DSS_nc_gather_rhs!(M, SD::NSD_2D, QT::Inexact, Mel::AbstractArray,
    ...
    for ieq in 1:neqs
        send_and_receive!(cache_ghost_p, @view(M_gather_ghost[:,ieq]))
        for (gip, m_value) in zip(gip_local, cache_ghost_p.combined_recv_data)
            M[gip2ip[gip], ieq] += m_value
        end
    end
end
...
function DSS_nc_scatter_rhs!(M, SD::NSD_2D, QT::Inexact,
                             non_conforming_facets,
                             non_conforming_facets_children_ghost, cache_ghost_c,
                             Ms, M_scatter_tmp, M_scatter_ghost,
                             IPc_list, IPp_list, IPp_list_cg,
    ...
    for (idx, ncf) in enumerate(non_conforming_facets)
        ...
        fill!(Ms, zero(TFloat))
        fill!(M_scatter_tmp, zero(TFloat))
        @turbo for j = 1:ngl
            ipp = IPp[j]
            Ms[j] = M[ipp]
            for i = 1:ngl
    ...
    send_and_receive!(cache_ghost_c, M_scatter_ghost)
    for (gip, m_value) in zip(gip_local, cache_ghost_c.combined_recv_data)
        M[gip2ip[gip]] = m_value
    end
end
...
function DSS_nc_gather_mass!(M, mesh, SD::NSD_3D, QT::Inexact, Mel::AbstractArray, conn::AbstractArray, conn_facets::AbstractArray,
            ...
            for i1 = 1:ngl
                ...
                for j2 = 1:ngl
                    for j1 = 1:ngl
                        j = j1 + (j2 - 1) * ngl
                        ipp = (idx-1) * ngl2 + j
                        M_gather_ghost[ipp] += Lt_1[j1,i1] * Lt_2[j2,i2] * Mg[i]
                    end
                end
            end
        end
    ...
    M_local = send_and_receive(M_gather_ghost, gip_gather_owner, comm)[1]
    gip_local = send_and_receive(gip_gather_ghost, gip_gather_owner, comm)[1]
    # @info "M_local", rank, [(mesh.coords[1,gip2ip[gip]], mesh.coords[2,gip2ip[gip]], gip) for gip in gip_local]
...
function DSS_nc_scatter_mass!(M, SD::NSD_3D, QT::Inexact, Mel::AbstractArray, conn::AbstractArray, conn_facets::AbstractArray,
    ...
    #     return
    # end
    M_scatter_ghost = KernelAbstractions.zeros(CPU(),  TFloat, num_p_ghost * (ngl2))
...
function DSS_nc_scatter_rhs!(M, SD::NSD_3D, QT::Inexact, Mel::AbstractArray, conn::AbstractArray, conn_facets::AbstractArray,
    ...
    # @info rank, length(M_scatter_ghost)
    send_and_receive!(M_scatter_local, M_scatter_ghost, gip_scatter_owner, comm)[1]
    # gip_local1 = send_and_receive(gip_scatter_ghost, gip_scatter_owner, comm)[1]
    for (gip, m_value) in zip(gip_local, M_scatter_local)
        M[gip2ip[gip]] = m_value
    end
end

function DSS_nc_scatter_rhs!(M, SD::NSD_3D, QT::Inexact,
                             non_conforming_facets,
                             non_conforming_facets_children_ghost, cache_ghost_c,
                             Ms, M_scatter_tmp, M_scatter_ghost,
                             IPc_list, IPp_list, IPp_list_cg,
...
function conformity4ncf_q!(q, q_el_tmp, q_tmp, vaux, g_dss_cache,
    ...
    if ladapt == true
        DSS_nc_gather_rhs!(q_tmp, SD, QT, q_el_tmp,
                           ...
                           q_el, q_el_pro, q_ghost_p,
                           mesh.IPc_list, mesh.IPp_list, mesh.IPc_list_pg,
                           mesh.ip2gip, mesh.gip2ip, mesh.pgip_ghost, mesh.pgip_local, ngl-1, neqs, interp)
    end
    DSS_global_RHS!(@view(q_tmp[:,:]), g_dss_cache, neqs)
    for ieq=1:neqs
        ...
        if ladapt == true
            DSS_nc_scatter_rhs!(@view(q_tmp[:,ieq]), SD, QT,
                                ...
                                q_el, q_el_pro, q_ghost_c,
                                mesh.IPc_list, mesh.IPp_list, mesh.IPp_list_cg,
                                mesh.gip2ip, mesh.cgip_local, ngl-1, interp)
        end
...
function redistributed_q!(u, u_old, glue_redistribute, connijk, old_connijk, ngl, SD::NSD_2D)
    ...
    u2send  = KernelAbstractions.zeros(CPU(), TFloat, Int64(num2send * ngl*ngl*neq))
    u2owner = KernelAbstractions.zeros(CPU(), TInt, Int64(num2send * ngl*ngl*neq))
    cnt = 1
    for (i, part) in enumerate(local_views(parts_snd).item_ref[])
        for idx in getindex(id2send, i)
            for l = 1:ngl
                for m = 1:ngl
                    ip = old_connijk[idx,l,m]
                    for ieq = 1:neq
    ...
    for (id, n2o_id) in enumerate(local_views(new2old).item_ref[])
        ...
        for l = 1:ngl
            for m = 1:ngl
                old_ip = old_connijk[n2o_id,l,m]
                new_ip = connijk[id,l,m]
                for ieq = 1:neq
