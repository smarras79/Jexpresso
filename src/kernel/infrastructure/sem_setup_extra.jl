...
function sem_setup_extra(inputs::Dict)
    
    fx = zeros(Float64,1,1)
    fy = zeros(Float64,1,1)
    fy_lag = zeros(Float64,1,1)
    Nξ    = inputs[:nop_extra]
    lexact_integration = inputs[:lexact_integration_extra]    
    PT    = inputs[:equations]
    AD    = inputs[:AD]
    CL    = inputs[:CL]
    ...
    # ω = ND.ξ.ω
    #--------------------------------------------------------
    mesh = mod_mesh_mesh_driver_extra(inputs)
    ...
    #--------------------------------------------------------
    ξω  = basis_structs_ξ_ω!(inputs[:interpolation_nodes_extra], mesh.nop, inputs[:backend])    
    ξ,ω = ξω.ξ, ξω.ω    
    if lexact_integration
        ...
        ξωQ   = basis_structs_ξ_ω!(inputs[:quadrature_nodes_extra], Qξ)
        ξq, ω = ξωQ.ξ, ξωQ.ω
    else  
        #
        # Inexact quadrature:
