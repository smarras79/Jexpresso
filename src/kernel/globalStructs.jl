...
Base.@kwdef mutable struct St_SolutionVars{TFloat <: AbstractFloat}
    ...
    #F    = Array{TFloat}(undef, 0, 0, 0) # Gⁿ[ngl,ngl,ngl,neqs]
    #G    = Array{TFloat}(undef, 0, 0, 0) # Hⁿ[ngl,ngl,ngl,neqs]
    #H    = Array{TFloat}(undef, 0, 0, 0) # Sⁿ[ngl,ngl,ngl,neqs]
    #S    = Array{TFloat}(undef, 0, 0, 0) # qelⁿ[ngl,ngl,ngl,neqs]
    neqs = UInt8(1)
