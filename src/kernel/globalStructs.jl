...
#-------------------------------------------------------------------------------------------
# Incompressible Navier-Stokes (vorticity-stream function) auxiliary structs
#-------------------------------------------------------------------------------------------
...
# Flux data: storage for quantities used in user_flux! that can't be obtained
# directly from the prognostic variables (e.g. ∇ψ for the vorticity-stream form).
...
function allocate_F_data(SD, npoin, T, backend; l_incompressible=false)
    ...
    F_data_st = St_F_data{T, dims1, backend}()
    return F_data_st
end

# Time-step / iteration counter used by the Poisson boundary update
...
# Poisson equation rhs and stiffness storage
...
# Boundary segment storage used when computing the Poisson boundary integral
