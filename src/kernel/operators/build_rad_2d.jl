...
function build_radiative_transfer_problem(mesh, inputs, neqs, ngl, dψ, ψ, ω, Je, dξdx, dξdy, dηdx, dηdy, nx, ny, elem_to_edge, 
                        ...
                        for iθ = 1:extra_meshes_extra_nops[iel][e_ext]+1
                            ...
                            if (ip in mesh.poin_in_bdy_edge)
                                ...
                                if (matchx && matchy) ##Do corner boundary case
                                    ...
                                    if (cos(θ) * (nx[iedge,edge_i]+nx[iedge_found,edge_found_i]) + sin(θ)*(ny[iedge,edge_i]+ny[iedge_found,edge_found_i]) <= 0.0) #&& (cos(θ) * nx[iedge_found,edge_found_i] + sin(θ)*ny[iedge_found,edge_found_i] < 0.0)
    ...
    #=if inputs[:adaptive_extra_meshes]
        @info "absolute errors, inf, L1, L2", maximum(abs.(solution_new - ref)), sum(abs.(solution_new-ref))/npoin_ang_total, sqrt(sum((solution_new-ref).^2))/npoin_ang_total
        @info "relative errors, inf, L1 ,L2", maximum(abs.(solution_new - ref))/maximum(abs.(ref)), sum(abs.(solution_new-ref))/sum(abs.(ref)), sqrt(sum((solution_new-ref).^2))/sqrt(sum((ref).^2))
        @info "norms", maximum(abs.(solution_new - ref)), maximum(abs.(ref)), sum(abs.(solution_new-ref)), sum(abs.(ref)), sqrt(sum((solution_new-ref).^2)), sqrt(sum((ref).^2))
    else
