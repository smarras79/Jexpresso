% =====================================================================
% Two-column table summarising the Jexpresso (Julia) -- Alya (Fortran)
% coupling. Each row pairs the matching snippets from the two codes that
% together implement one step of the coupling protocol. Snippets are
% lifted verbatim (lightly trimmed) from:
...
\newcommand{\ftn}[1]{\lstinline[style=fortran]{#1}}
\newcommand{\jul}[1]{\lstinline[style=julia]{#1}}
...
\toprule
\textbf{Step} & \textbf{Alya (Fortran)} & \textbf{Jexpresso (Julia)} \\
\midrule

% --------------------------------------------------------------- 1
\textbf{1. MPI init \&\\ communicator split}
&
...
% --------------------------------------------------------------- 2
\textbf{2. Handshake:\\ application names}
&
\begin{lstlisting}[style=fortran]
app_name = 'ALYA'
...
% --------------------------------------------------------------- 3
\textbf{3. Grid metadata\\ broadcast\\ (Alya $\rightarrow$ Julia)}
&
\begin{lstlisting}[style=fortran]
...
% --------------------------------------------------------------- 4
\textbf{4. Alya-to-world\\ rank map\\ (Allreduce)}
&
\begin{lstlisting}[style=fortran]
...
% --------------------------------------------------------------- 5
\textbf{5. Point-count\\ exchange\\ (MPI\_Alltoall)}
&
\begin{lstlisting}[style=fortran]
...
% --------------------------------------------------------------- 6
\textbf{6. Send/Recv of\\ Jexpresso SEM\\ node global IDs}
&
\begin{lstlisting}[style=fortran]
do i = asize, size-1
...
% --------------------------------------------------------------- 7
\textbf{7. Time loop:\\ field receive\\ (Julia $\rightarrow$ Alya)}
&
\begin{lstlisting}[style=fortran]
do step = 1, nsteps
   ireq        = 0
...
% --------------------------------------------------------------- 8
\textbf{8. Time loop:\\ scatter into\\ ordered buffer}
&
\begin{lstlisting}[style=fortran]
...
collectives. The collective order in rows 3--5 is identical on the two
