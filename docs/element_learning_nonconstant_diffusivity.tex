...
\subsection{The \code{:lEL\_xidependent} flag}
This is a \textbf{sampling-only} option that controls how the synthetic
training $\ahat$ is generated. The symbol $\xi$ is the reference-element
coordinate, so ``$\xi$-dependent'' means $\ahat$ \emph{varies within} the
element (node to node) rather than being a single value per element.

\begin{center}
\begin{tabularx}{\linewidth}{@{}llY@{}}
\toprule
\code{:lEL\_xidependent} & sampler & each training element's $\ahat$ \\
\midrule
\code{false} (default) & \code{synthesize\_random\_ahat} --- random
  \textbf{affine} Jacobian (rotation $\times$ stretch $\times$ shear) &
  \textbf{constant within the element} (parallelogram); all $(k+1)^2$ nodes
  identical \\
\code{true} & \code{synthesize\_random\_ahat\_field} --- smooth anisotropic
  per-node warp & \textbf{varies within the element} (mimics a bilinear /
  curved element) \\
\bottomrule
\end{tabularx}
\end{center}
...
affine sampler only ever produces \emph{constant-per-element} feature vectors,
so a model trained on it never sees within-element variation. Set
\code{:lEL\_xidependent => true} so the training data resembles what inference
actually feeds. The flag has \textbf{no effect at inference} --- inference always
builds $\ahat$ from the real mesh metrics.
...
:lEL_xidependent  => true,   # within-element-varying a-hat (recommended)
...
size $3(k+1)^2$ and output size $n_{vo}n_{vb}$ depend on it. (If instead you want
the samples drawn from the \emph{actual} hole element shapes, use the full circle
mesh together with the bilinear-quad sampler noted at the end of this section.)
Inference/verification then uses the full circle mesh.
...
The samples are \textbf{synthetic} (Option~1: synthesised random Jacobians).
...
\textbf{verification at inference}, not data generation. To instead draw each
sample from the \emph{actual} mesh element shapes, a bilinear-quad sampler (draw
4 random corners, build the bilinear map, take per-node $\ahat$) is the natural
extension.
