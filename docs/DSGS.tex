...
viscous contribution of Section~\ref{sec:fluxes} is assembled separately
into \code{params.RHS\_visc}, added, and the sum is divided by $M$ to give
$\partial_t\bq_h$.
...
\subsection{Time derivative: a step-cadenced BDF2}

\dsgs\ keeps its own pair of state snapshots, $\bq^{n-1}$ and $\bq^{n-2}$
(\code{params.dsgs\_qnm1}, \code{dsgs\_qnm2}), advanced \emph{once per time
step}: inside \code{rhs!}, when the stage time satisfies
$t - t_{\rm hist} \ge 0.999\,\Delta t$, the pair is rolled
($\bq^{n-2}\leftarrow\bq^{n-1}$, $\bq^{n-1}\leftarrow\bq_h$) and
...
\begin{equation}
\boxed{\;
R^{q}_i \;=\; \Big|\;\frac{3q_i - 4q^{\,n-1}_i + q^{\,n-2}_i}{2\Delta t}
\;-\; \frac{\mathrm{RHS}_i^{\,q}}{m_i}\;\Big| \;}
\qquad i = 1,\dots,N_h,
\label{eq:residual}
\end{equation}
i.e.\ the absolute value of the \emph{nodal} value of the lumped-mass
strong residual $|\mathrm{BDF2}(q) + \nabla\!\cdot\!\bF^q - S^q|$. The
absolute value is taken \emph{after} the assembly and the division by $m_i$.
...
\label{eq:Sbar}
\end{equation}
With \code{:dsgs\_norms => "domain"} both are reduced over all MPI ranks
(the count $|\mathcal P|$, the sum, then the max); with \code{"rank"} they
are taken over the rank's own elements without communication.

The spread $\Sbar^q$ is then floored at a fraction $r = 10^{-3}$ of a
physical scale of the slot built from the mean state
...
and the floors~\eqref{eq:floors} are applied with the \emph{element-mean}
...
\end{equation}
reduced over the ranks with \code{"domain"}, and $\Sbar^q$ is floored as
in~\eqref{eq:floors} \emph{without} the trailing $+\epsilon$. For every node
...
operation is the background floor; with $\Cmin=0$ it is absent and
...
%======================================================================
\section{The Euler kernels (\code{DSGS})}
\label{sec:euler}
...
Section~\ref{sec:norm-domain} with the floors replaced by $n^q = \Sbar^q+\epsilon$
...
\end{equation}
with $\mathrm{Pr}$ = \code{:Pr} (the decks use $0.1$). The primitives are
$(\rho,u,v,\theta)$.
...
floored as in~\eqref{eq:floors} for $\rho$, $m$, $E$. With
...
T:\ \mu_4\,\frac{\mathrm{Pr}}{\gamma-1}\,\mu_K ,
\end{equation}
...
$n^q=\Sbar^q+\epsilon$ (no physical floors), $\Delta=\Delta_K/n_{gl}$,
...
\code{:dsgs\_norms} & \code{"domain"} & scope of $\langle q\rangle,\Sbar^q$: \code{"domain"}, \code{"rank"}, \code{"element"} \\
...
\code{:Pr} & -- & $\mathrm{Pr}$ of the Euler kernels \\
\bottomrule
\end{tabularx}
\end{center}
...
Lumped (diagonal) $M$ everywhere, including the residual \\
...
Identical stencil, eq.~\eqref{eq:residual}; $q^{n-1},q^{n-2}$ are once-per-step snapshots, $q$ the current RK stage state \\

Residual $R(q)$ &
Lumped $L^2$ projection of the absolute value: $R_i = \frac1{m_i}\int|\mathrm{BDF2}+\nabla\!\cdot f|\,\phi_i$ (eq.~4.9) &
Absolute value of the nodal lumped residual: $R_i = |\mathrm{BDF2}_i - \mathrm{RHS}_i/m_i|$, the assembled inviscid RHS (fluxes and sources) divided by $m_i$ \\
...
$\Sbar^q$ floored at $10^{-3}$ of the slot's physical scale~\eqref{eq:floors} from the mean state; then the same guard in the nodal form, $R/(n+\epsilon)$ in the element form \\
