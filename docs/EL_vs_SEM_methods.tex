...
For SEM the timed call is \verb|A\b|, which for a sparse $A$ computes a fresh LU
factorization and then back-substitutes on every sample. For EL the timed call is
the whole condensation solve \verb|elementLearning_Axb!|: extract the per-element blocks
...
cached factorization for a new right-hand side. For SEM, $F=\mathrm{lu}(A)$ is
pre-computed and the timed call is only the back-solve \verb|F\b| (the full
system). For EL, having already assembled the surrogate operators $\{T^e\}$ and the
Schur complement $S$ during the solve above, $F_S=\mathrm{lu}(S_{\partial O,\partial O})$
is pre-computed and the timed call is only the back-solve $F_S\backslash\mathbf{b}$
--- the back-substitution of the
...
\paragraph{Where the LU factorization is done.} Solving a sparse system
$A\mathbf{x}=\mathbf{b}$ directly means factorizing $A=LU$ into triangular factors
and then performing two cheap triangular solves (forward $L\mathbf{y}=\mathbf{b}$,
back $U\mathbf{x}=\mathbf{y}$). The factorization is the expensive step; the
back-substitutions are $O(\mathrm{nnz})$. In Jexpresso this LU is computed
\emph{implicitly} by the backslash operator (sparse UMFPACK): it is not a separate
line of code but part of every \verb|A\b|, and --- since backslash does not cache
--- it is recomputed on each call. The direct SEM solver factorizes the
\emph{full} matrix $A$. Element learning never factorizes $A$: its only global
factorization is the LU of the \emph{condensed skeleton} matrix
$S_{\partial O,\partial O}$ \eqref{eq:schur}, which is far smaller than $A$,
together with small \emph{dense} inverses $(\Aoo^e)^{-1}$ of the per-element
interior blocks used to form the local operators $T^e,t^e$ \eqref{eq:Te}. The
amortized measurement is exactly what separates these two costs: it caches each
method's factorization ($F=\mathrm{lu}(A)$ for SEM, $F_S=\mathrm{lu}(S)$ for EL)
and times only the triangular back-solve for a new right-hand side.

\begin{table}[h]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{@{}>{\hsize=0.7\hsize}Y >{\hsize=1.15\hsize}Y >{\hsize=1.15\hsize}Y@{}}
\toprule
