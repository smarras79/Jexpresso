...
\paragraph{Amortized (setup reused).} The per-solve cost once the $A$-dependent
setup has been done once and cached. \emph{SEM} pre-computes the factorization
$F=\mathrm{lu}(A)$ \emph{outside} the timer and times only \verb|F\b| --- the
triangular back-substitution alone. \emph{EL} runs \verb|elementLearning_Axb!| once
outside the timer (which caches the extracted element blocks) and then times
\verb|elementLearning_infer!|; this reuses the extracted blocks but still, on every
sample, re-evaluates the surrogate, re-assembles $S$, re-solves the skeleton system,
and recovers the interiors.
...
\textbf{Amortized} \verb|F\b| / \verb|infer!| & back-solve, $F=\mathrm{lu}(A)$ cached & surrogate + assemble $S$ + skeleton solve + recover (blocks cached) \\
\bottomrule
\end{tabular}
\caption{What is inside each timed call. The two methods hoist \emph{different}
amounts of work in the amortized row: SEM reuses the \emph{entire} factorization, so
only the minimal right-hand-side-dependent back-substitution is timed; EL reuses
\emph{only} the per-element block extraction, so its surrogate, Schur assembly, and
skeleton solve are still repeated --- even though $\{T^e\}$ and $S$ depend on $A$
alone and could themselves be cached. The EL amortized figure is therefore a
\emph{conservative upper bound} on a fully-optimized repeated-solve cost (which would
cache $S$ and its factorization and per-RHS only form the condensed load,
back-solve $S$, and recover), whereas the SEM figure already attains that level.}
\label{tab:cost}
\end{table}
