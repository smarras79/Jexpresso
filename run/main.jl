...
function get_global_indices_periodic(element_id, poly_deg, n_dof_global)
    ...
    # Applica periodicità: l'ultimo nodo torna al primo
    indices = mod1.(indices, n_dof_global)
    return indices
end

# Assembly della matrice globale con PBC
for k = 1:n_elements
...
function rhs_local!(du, u, parameters, t)
    ...
    for elem in 1:n_elements
        ...
        for i_loc in 1:n_nodes_per_element
            ...
            # Accumula (importante per nodi condivisi tra elementi)
...
n_elements = 12
n_dof_global = 36
jacobian = (2/ n_elements) / 2  # o L/n_elements/2
