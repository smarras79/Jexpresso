...
The skeleton matrix B is symmetric in exact arithmetic, but the subtraction that forms it leaves a round-off asymmetry (≈1e-16 relative). That was enough for Julia's `factorize`, which tests for exact symmetry, to fall back to UMFPACK LU. `el_skeleton_solve` now symmetrises B as (B + Bᵀ)/2 and factorises it with CHOLMOD Cholesky, like the full system. The table and figures below use Cholesky. The high-resolution section further down was measured earlier, with LU for SC direct and with the previous test problem, the trigonometric polynomial $u = \sin 2x\cos 3y + \sin x\cos y$.
