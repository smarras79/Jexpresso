...
| `log10_μ_dsgs_ρ-it<n>.png`, `log10_μ_dsgs_ρE-it<n>.png` | $\log_{10}$ of the DynSGS coefficients actually applied (kinematic, $H_0C_s$, floored at $10^{-6}$): $\nu$ on the $\rho$, $\rho\mathbf{v}$, $\mathbf{B}$, $\psi$ slots and, on the thermal part of the energy, $\max(\gamma(\gamma-1)/\mathrm{Pr}\cdot\nu_{res}, \nu_{floor})$, the coefficients by equation of Dao & Nazarov (2022) (`:dsgs_nazarov_energy`, DSGS.md §4.5; the kinetic and magnetic part of $E$ is diffused with $\nu$) | — |
...
(`ρ u v w p Bx By Bz ψ T vA β`) plus the DynSGS coefficients (one field per distinct coefficient) to
