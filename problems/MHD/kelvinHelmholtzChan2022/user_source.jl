...
# dissipation. In the paper's DG setting the interface Riemann fluxes
# upwind-damp those waves; Jexpresso's continuous-Galerkin weak form has no
# interface dissipation, so on a doubly periodic domain the ψ waves keep
# bouncing, alias into grid-scale (LGL odd-even) oscillations and slowly
# grow, while every other field stays clean.
#
...
#
# NOTE: the non-conservative Galilean GLM transport (v·∇ψ) of the paper's
# Eq. (7) is still omitted; see user_flux.jl.
#---------------------------------------------------------------------------------
