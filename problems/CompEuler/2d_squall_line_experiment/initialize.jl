    ...
    if (inputs[:backend] == CPU())
        ...
        if inputs[:lrestart] == true
            ...
            for ip = 1:mesh.npoin
                ...
                θ      = θ_ref + Δθ
                θv     = θv_ref + Δθ
                p      = PhysConst.pref*(1.0 - PhysConst.g*y/(PhysConst.cp*θv))^(PhysConst.cpoverR) #Pa
        ...
        k(q.qn, q.qe, background, view(mesh.coords, 1, :), view(mesh.coords, 2, :), view(mesh.coords, 3, :), xc, rx, ry, yc, θc, PhysConst, lpert; ndrange = (mesh.npoin))
    end
...
@kernel function initialize_gpu!(qn, qe, background, x, y, z, xc, rx, ry, yc, θc, PhysConst, lpert)
    ...
    r = T(sqrt( (x1 - xc)^2/(rx^2) + (y1 - yc)^2/(ry^2) ))
    ...
    if (lpert)
        ...
        qn[ip,2] = ρ*u - ρref*u
        qn[ip,3] = ρ*v - ρref*v
        qn[ip,4] = ρ*hl - ρref*hl_ref
        qn[ip,5] = ρ*qv_ref - ρref*qv_ref
        qn[ip,6] = T(0.0)
        qn[ip,end] = pref_m
    else
        qn[ip,1] = ρ
