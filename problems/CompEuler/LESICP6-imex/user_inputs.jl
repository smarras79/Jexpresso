#==============================================================================
 LESICP6-imex -- LESICP6 with all three time integrators behind one switch.

 A copy of problems/CompEuler/LESICP6. Physics, closure, wall model, filter,
 ...
 output directory, and the per-scheme option blocks below.
   ...
   for s in explicit hevi imex; do
     DBG_SCHEME=$s mpiexecjl -n 16 julia --project=. \
       -e 'using Jexpresso; Jexpresso.run_case("CompEuler","LESICP6-imex")'
   done
...
function user_inputs()
    ...
    # case behaves exactly like LESICP6 until you ask for something else.
    #---------------------------------------------------------------------------
    scheme = Symbol(get(ENV, "DBG_SCHEME", "explicit"))
    scheme in (:imex, :hevi, :explicit) ||
        error("LESICP6-imex: DBG_SCHEME must be imex, hevi or explicit; got $scheme")
	...
	:mu_z                => 0.1,
        ...
        :output_dir          => "/scratch/smarras/hw59/output_new/LESICP6_64x64x36_10kmX10kmX3dot5km-smag15_" * String(scheme) * "/",
        #:output_dir          => "./output",
