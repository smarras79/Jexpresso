...
const MSH = joinpath(CASE_DIR, "b747_highlift.msh")
...
function main()
    ...
    for want in ("bottom", "outflow", "top", "inflow", "domain")
    ...
    #---------------------------------------------------------------------------------
    # The elements, and the sections the deck places. One physical group each.
    #---------------------------------------------------------------------------------
    function deck_elements(file)
        out = Tuple{String,NTuple{5,Float64}}[]
        for line in eachline(file)
            code = lstrip(line)
            startswith(code, "#") && continue
            m = match(r"\"([A-Za-z0-9_]+)\"\s*=>\s*\(:naca64A210((?:\s*,\s*[-\d.eE+]+){5})\s*\)", code)
            m === nothing && continue
            v = [parse(Float64, strip(x)) for x in split(m.captures[2], ",")[2:end]]
            push!(out, (m.captures[1], (v[1], v[2], v[3], v[4], v[5])))
        end
        isempty(out) && error("check_mesh.jl: no (:naca64A210, ...) entries in " * file)
        return out
    end
    ELEMS = deck_elements(joinpath(CASE_DIR, "user_inputs.jl"))
    for (nm, _) in ELEMS
        nm in have || error("check_mesh.jl: the deck names element \"$nm\" but the mesh " *
                            "has no physical group of that name.")
    end

    walls = Dict{String,Vector{Tuple{Int,Int}}}()
    secs  = Dict{String,Any}()
    for (nm, pl) in ELEMS
        ph = only([t for (t, n) in names if n == nm])
        es = Set(t for (t, ps) in curvephys if ph in ps)
        w  = [(e[2], e[3]) for e in edges if e[1] in es]
        isempty(w) && error("check_mesh.jl: no boundary edge carries the \"$nm\" tag.")
        walls[nm] = w
        secs[nm]  = naca64A210_section(pl[1], pl[2], pl[3], pl[4], pl[5])
        @printf("  %-6s chord %.4g, LE (%.4g, %.4g), %+.4g deg, TE r = %.4g m ; %d wall edges\n",
                nm, pl[3], pl[1], pl[2], -pl[4], secs[nm].te.r, length(w))
    end

    #---------------------------------------------------------------------------------
    # THE SLOTS. On a multi-element section these are the flow, and a slot two
    # cells wide is not a slot. Measured between the boundary NODES, which is
    # what the solver sees.
    #---------------------------------------------------------------------------------
    nodesof(nm) = begin
        S = Set{Int}(); for (a,b) in walls[nm]; push!(S,a); push!(S,b); end; collect(S)
    end
    println("  slot gaps (element to element, and elements across each):")
    for i = 1:length(ELEMS), j = i+1:length(ELEMS)
        A, B = nodesof(ELEMS[i][1]), nodesof(ELEMS[j][1])
        d = Inf
        for a in A, b in B
            dd = (coord[a][1]-coord[b][1])^2 + (coord[a][2]-coord[b][2])^2
            dd < d && (d = dd)
        end
        d = sqrt(d)
        d > 0.25 && continue                      # not neighbours; nothing to report
        # local element size = mean wall-edge length of the two
        h = 0.0; n = 0
        for nm in (ELEMS[i][1], ELEMS[j][1]), (a,b) in walls[nm]
            h += hypot(coord[b][1]-coord[a][1], coord[b][2]-coord[a][2]); n += 1
        end
        h /= n
        @printf("     %-6s / %-6s  gap = %.4f m   ~%.1f elements%s\n",
                ELEMS[i][1], ELEMS[j][1], d, d/h,
                d/h < 4 ? "   <-- THIN, the slot is under-resolved" : "")
    end
    ...
    @printf("     2. largest wall-edge sagitta: %.3e at (%.4f, %.4f), edge %.3e\n",
            worst_sag, at_sag[1], at_sag[2], hr_sag)
    @printf("        smallest wall edge       : %.3e\n", hmin)
    @printf("        worst thickness/sagitta  : %.1f at (%.4f, %.4f)%s\n",
            worst_ratio, at_ratio[1], at_ratio[2],
            worst_ratio < 3 ? "   <-- THIN. Elements may fold; refine normal to the wall." :
                              "   (fold margin, wants >> 1)")
    (worst_ratio > 3 && off <= 0.01*chord) || (allok = false)
    end   # per-element loop

    println(allok ?
            "\n  OK — every element can be curved onto its section." :
