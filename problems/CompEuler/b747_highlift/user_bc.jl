...
#   "slat"     leading-edge slat                   free slip
#   "main"     main element                        free slip
#   "flap1"    first trailing-edge flap            free slip
#   "flap2"    second trailing-edge flap           free slip
#
# The four aerofoil elements need no separate handling: everything that is not
# inflow or outflow falls through to the free-slip branch, so adding an element
# to the configuration is a mesh-and-deck change, not a boundary-condition one.
#
...
#
# WHY THE AEROFOIL WALLS ARE WHERE THE CURVED GEOMETRY PAYS. The slip projection
...
#                                      wall acts.
#
# The aerofoil trailing edges would have been the exception — a cusp is one node
# shared by two boundary edges whose normals are ~168° apart, so the two
# successive projections there are fighting each other — but every element's
# trailing edge is ROUNDED (see user_exactGeo.jl). The largest turn of the wall
# normal between adjacent boundary edges is now at the leading edges, and it is
# the ordinary O(h) rotation of a smooth curve, not a jump.
#---------------------------------------------------------------------------------
...
function user_bc_dirichlet!(q, coords, t::AbstractFloat, tag::String,
    ...
    elseif tag == "outflow"
        ...
        # tunnel walls and all four aerofoil elements: free slip. Remove the
        # wall-normal momentum component.
        qnl     = nx*q[2] + ny*q[3]
        qbdy[2] = q[2] - qnl*nx
        qbdy[3] = q[3] - qnl*ny
    end
