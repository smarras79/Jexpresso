...
# Identical to problems/CompEuler/ffs_step, from which this case takes the
# 2D total-energy Euler set, except that the output temperature is
# nondimensional (see user_uout! below).
#
...
function user_uout!(ip, ET, uout, u, qe; kwargs...)
    ...
    # THIS CASE IS NONDIMENSIONAL (ρ∞ = p∞ = T∞ = 1, R = 1), so the output
    # temperature is p/ρ and not p/(R ρ) in kelvin: dividing by Rair here
    # would report 1/287 for the free stream and make the field unreadable.
    uout[5] = uout[4]/ρ                                      # T = p/ρ
end
