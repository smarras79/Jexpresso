...
function user_source!(S, q, qe, npoin, ::CL,::PERT; neqs=1,x=0.0, y=0.0, ymin=0.0, ymax=30000.0, ngl=5, nely=10,xmin=-50000.0, xmax=50000.0)
    ...
    # SPONGE
    #--------------
    zs = 20000.0       # sponge starts at 20km (top 10km of 30km domain)
    xr =  40000.0      # 10km lateral sponge on right
    xl = -40000.0      # 10km lateral sponge on left
    ...
    ctop = 0.1*min(ctop, 1)
    cxr  = 0.1*min(betaxr_coe, 1)
    cxl  = 0.1*min(betaxl_coe, 1)
    cs   = 1.0 - (1.0-ctop)*(1.0-cxr)*(1.0-cxl)
...
function user_source_gpu(q,qe,x,y,PhysConst,xmax,xmin,ymax,ymin,lpert)
    ...
    #xr = 140000.0   # 20km from right boundary (160km - 20km)
#xl = 20000.0    # 20km from left boundary
