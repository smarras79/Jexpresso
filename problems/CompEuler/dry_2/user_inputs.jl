function user_inputs()
    inputs = Dict(
       ...
       # :nop_laguerre        => 24,
        ...
        #:visc_model  => SMAG():μ           => [0.0, 300.0, 300.0, 300.0], #horizontal viscosity constant for momentum
        #:μ           => [0.0, 20.0, 20.0, 20.0, 30.0, 30.0], #horizontal viscosity constant for momentum
        :μ           => [0.0, 200.0, 200.0, 0.0], #horizontal viscosity constant for momentum
       #:μ           => [0.0, 300.0, 300.0, 300.0], #horizontal viscosity constant for momentum
        #---------------------------------------------------------------------------
        #---------------------------------------------------------------------------
        ...
        :mount_type          => "agnesi",
        :a_mount             => 10000.0,
        :h_mount  => 1.0, #2500.0,   # currently 100.0 — needs to be 2.5 km
        ...
        :sounding_file       => "./data_files/no_shear.data", #no_shear data.
       # :sounding_file       => "./data_files/test_hill.data", #main sounding data
      #   :sounding_file       => "./data_files/test_hill_hydro_noqv.data", 
       # :sounding_file       => "./data_files/input_sounding", #test_hill.data minus qv
        ...
        #---------------------------------------------------------------------------
        :lfilter             => true,
        :mu_x                => 0.25, #0.25 according to paper
        :mu_y                => 0.25,
