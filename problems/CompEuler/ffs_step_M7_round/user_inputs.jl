...
function user_inputs()
    ...
    inputs = Dict(
        #---------------------------------------------------------------------------
        #
        # 2D CompEuler: Mach-7 forward-facing step, FILLETED CORNER.
        #
        # THIS IS CompEuler/ffs_step_M7 WITH THE STEP CORNER ROUNDED, and
        # nothing else changed. Same free stream, same fluxes, same boundary
        # conditions, same DynSGS settings, same Δt, same tend, same sweep
        ...
        # SINGULARITY is what kills the Mach-7 step.
        #
        ...
        # unable to help there, which is why every knob on ffs_step_M7 bought
        # a factor and none of them fixed it, and why a COMPLETE removal of
        # the top mode by the modal filter (μ_x = 0.005) moved the death only
        # from 7.5e-4 to 1.1e-3 while the structures downstream of the corner
        # survived. Woodward & Colella (1984) section IV call this corner "a
