...
function user_source!(S, q, qe, npoin, ::CL,::PERT; neqs=1,x=0.0, y=0.0, ymin=0.0, ymax=24000.0, ngl=5, nely=10,xmin = 0.0, xmax =80000)
    ...
    cxr = 0.002*betaxr_coe
    cxl = 0.002*betaxl_coe
    ctop = 0.02*min(ctop,1)
    cxr  = min(cxr,1)
    cxl  = min(cxl,1)
    cs = 1.0 - (1.0 -ctop)*(1.0-cxr)*(1.0 - cxl)
