function initialize(SD::NSD_3D, PT, mesh::St_mesh, inputs::Dict, OUTPUT_DIR::String, TFloat)
    ...
    if inputs[:lrestart] == true
        ...
        if lanalytic == true
            ...
            # Spin-up probe switches (see the deck header). Defaults reproduce
            # the 64x64x60 deck exactly.
            amp     = parse(Float64, get(ENV, "DBG_AMP", "0.25"))
            ...
            for ip = 1:mesh.npoin
                ...
                if loginit && mesh.z[ip] < logz
                    # log law from z0 to logz, matching the sounding at logz,
                    # so there is no shear layer sitting on the wall at t = 0
                    fac = log(max(mesh.z[ip], z0m)/z0m) / log(logz/z0m)
                    u *= fac; v *= fac
                end
                w     = 0.0
