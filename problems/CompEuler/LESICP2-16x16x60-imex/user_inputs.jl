#=============================================================================
 LESICP2-16x16x60-imex -- SPIN-UP PROBE at the 64x64x60 resolution.

 The 64x64x60 deck (160 m elements, 10 km box) dies at t ~ 600 s: the initial
 wall shear layer rolls up from ~450 s and cannot break into 3D turbulence,
 the wall node is dragged in, and rho*theta goes negative. The 30x30 deck at
 341 m does the same; at 80 m it saturates and runs. This deck is the 64x64
 deck with a 2560 m box (256 columns, 64 ranks at 2 x 2 columns each), so a
 700 s probe costs ~2 h instead of 4, and the candidate fixes are switches:

     DBG_AMP=1.0      initial theta perturbation amplitude (default 0.25 K)
     DBG_LOGINIT=1    log-law wind below DBG_LOGZ (default 150 m) instead of
                      the sounding's uniform 10 m/s down to the wall
     DBG_MU=0.15      horizontal erf filter strength (default 0.05)

 Judge an arm on the wall-watch summary at t = 400-800: node2-layer max|uh|
 must peak and fall (80 m: 11.7 at 550, then down), not climb past ~14.
=============================================================================#
#=============================================================================
...
function user_inputs()
	...
	:gmsh_filename    => "./problems/CompEuler/LESICP2-128x128x60-imex/LESICP_128x128x60_10240mX10240mX5000m.msh",
        :gmsh_filename    => "./problems/CompEuler/LESICP2-16x16x60-imex/LESICP_16x16x60_2560mX2560mX5000m.msh",
	...
	:output_dir          => joinpath(get(ENV, "JEXPRESSO_OUTDIR",
	                                 ...
	                                 "LESICP2_16x16x60_2560mX2560mX5000m_" * String(scheme)) * "/",
