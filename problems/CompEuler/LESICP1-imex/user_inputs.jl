#==============================================================================
 LESICP1-imex -- LESICP1 with all three time integrators behind one switch.

 A copy of problems/CompEuler/LESICP1. Physics, closure, wall model, filter,
 ...
 output directory, and the per-scheme option blocks below.
   ...
   for s in explicit hevi imex; do
     DBG_SCHEME=$s mpiexecjl -n 16 julia --project=. \
       -e 'using Jexpresso; Jexpresso.run_case("CompEuler","LESICP1-imex")'
   done
...
function user_inputs()
    ...
    # case behaves exactly like LESICP1 until you ask for something else.
    #---------------------------------------------------------------------------
    scheme = Symbol(get(ENV, "DBG_SCHEME", "explicit"))
    scheme in (:imex, :hevi, :explicit) ||
        error("LESICP1-imex: DBG_SCHEME must be imex, hevi or explicit; got $scheme")
	...
	#:gmsh_filename    => "./meshes/gmsh_grids/LESICP_64x32x36_10kmX5kmX3km.msh",
        ...
        :output_dir          => "/scratch/smarras/hw59/output_new/LESICP1_scaling-8nodes-64x64x36_10kmX10kmX3km_" * String(scheme) * "/",
        #:output_dir          => "./output",
