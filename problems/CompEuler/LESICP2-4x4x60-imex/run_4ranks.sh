#!/bin/bash
#=============================================================================
#  LESICP2-4x4x60-imex on 4 MPI ranks, on a laptop. Run from the REPO ROOT:
#
#      ./problems/CompEuler/LESICP2-4x4x60-imex/run_4ranks.sh
#      DBG_TEND=10 ./problems/CompEuler/LESICP2-4x4x60-imex/run_4ranks.sh
#      DBG_VDIFF=0 ./problems/CompEuler/LESICP2-4x4x60-imex/run_4ranks.sh   # Schur arm
#
...
echo "--- Launching $NR ranks: CompEuler / LESICP2-4x4x60-imex ---"
julia --project=. --startup-file=no -e '
    using MPI
    nr = ARGS[1]
    MPI.mpiexec() do exe
        run(`$exe -n $nr julia --project=. --startup-file=no src/Jexpresso.jl CompEuler LESICP2-4x4x60-imex`)
    end' "$NR"
