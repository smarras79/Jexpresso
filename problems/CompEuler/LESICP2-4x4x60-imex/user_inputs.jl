#=============================================================================
 LESICP2-4x4x60-imex -- 16 COLUMNS, PRODUCTION ELEMENT SIZE, NO DEGENERATE
                        PERIODIC DIRECTION.
 ...
 in a few hours rather than days. UNLIKE ITS PREDECESSORS THIS ONE CAN SPEAK
 TO STABILITY, and that is the entire reason it exists.

 WHAT THE EARLIER LAPTOP DECKS GOT WRONG. LESICP2-4col-imex and
 LESICP2-16col-imex both shrank the COLUMN COUNT to fit a laptop and let the
 element size follow -- 640 m and 1000 m against production's 160 m. Under
 Smagorinsky nu_t = (C_s*Delta)^2*|S|, so the filter width enters SQUARED, and
 that made them 16x and 29x more diffusive than the case they were standing in
 for. All three runs died on a negative rho*theta with no CFL or wedge limit
 exceeded, and the COARSER of the two died SOONER:

     deck    element   filter width   dt     died at   nu_sgs before death
     4col      640 m       324 m      0.5      5550           36
     4col      640 m       324 m      0.25     7725          ~23
     16col    1000 m       437 m      0.25     3050           72.9

 The eddy viscosity that killed them is an artifact of a 320-440 m filter
 width. It says nothing about a run at 100 m.

 SO THIS DECK FIXES THE ELEMENT SIZE AND SHRINKS THE DOMAIN INSTEAD. 160 m
 elements are LESICP2-64x64x60-imex's, giving the same 100.8 m filter width
 and therefore the same nu_t for the same flow. What it gives up is domain
 size -- 640 x 640 m holds a couple of eddies, not a boundary layer -- so
 read it for STABILITY, never for statistics.

 THE GRID, AND WHY IT IS EXACTLY THIS SHAPE
 ------------------------------------------
   4 x 4 x 60 elements at p = 4 over 640 x 640 x 5000 m
   element 160 x 160 m -> 40 m effective resolution, AS IN LESICP2-64x64x60
   smallest LGL gaps  h_x = h_y = 27.63 m   h_z = 6.907 m   (anisotropy 4:1)

 WHY 4 x 4 AND NOT 8 x 2, WHICH IS THE POINT OF THIS DECK. Its three
 predecessors were 2 x 2, 8 x 2 and 8 x 2 -- every one of them had
 nelemy = 2 -- and all three died on a non-positive rho*theta with no CFL,
 wedge or viscous limit exceeded. A probe in user_flux.jl caught the first bad
 node in the 8 x 2 run:

     position  (480.0, 0.0, 40.0) m       <-- y = 0.0 IS THE PERIODIC FACE
     rho       0.846                       (healthy)
     rho*theta -524      theta = -619
     u, v, w   (-21.1, -0.77, -153.3)      <-- w = -153 m/s

 rho is fine, so this is not a thermodynamic drift: it is a VERTICAL MOMENTUM
 explosion, and theta going negative is its consequence. It happened on the
 y-periodic face, at z = 40 m, which is the top face of the first vertical
 element.

 nelemy = 2 WITH A PERIODIC y IS DOUBLY CONNECTED: element 1's -y face meets
 element 2's +y face AND element 1's +y face meets element 2's -y face, so the
 two elements are each other's neighbour on both sides. Nothing says the
 periodicity restructuring and the DSS were built for that, and the location
 of the first failure is exactly where it would show. 4 x 4 removes the
 degeneracy in both directions while keeping 16 columns, 4 per rank.

 IF THIS DECK SURVIVES PAST t = 660 s, EVERY CONCLUSION DRAWN FROM THE THREE
 PREDECESSORS IS VOID -- including "the Schur arm blows up here", which was
 measured on 2 x 2.
   71 577 gridpoints total, 17 894 per rank on 4 ranks -- EXACTLY EQUAL

 WHY 16 COLUMNS AND NOT 4. Its 2 x 2 predecessor gave each rank ONE column,
 the minimum that exercises the partition -- but then every element sits on a
 partition boundary. 8 x 2 hands each rank a 2 x 2 BLOCK, so there are
 interior elements as well as halo ones, which is a more representative test
 of the same machinery at the same laptop cost.

 h_x SHRANK, SO THIS TIME THE STEP LIMIT DOES MOVE. The explicit half is
 advection, uref*(1/h_x + 1/h_y + 1/h_z). At h_x = 27.63 m the two horizontal
 terms are 0.0362 each against h_z's 0.1448, so the sum goes from 0.1564 to
 0.2172 -- about 39% larger, and the wedge limit shrinks by the same factor.
 Expect roughly 0.30 s at :imex_umax = 30 where the 1000 m deck reported
 0.4186. Read the run's own "neutral up to" line rather than this estimate.
 ...
 divides exactly one way that uses four ranks:
     ...
     julia tools/pick_nranks.jl 4 4 60 4 8

 pick_nranks recommends 1 rank because it optimises for EFFICIENCY -- it wants
 >= 50 000 points/rank and this whole mesh is 71 577. That is the right answer
 for a production run and the wrong one here: this deck is a correctness test,
 and 4 ranks x 4 columns is a perfectly balanced decomposition. Run it on 4.

     mpiexec -n 4 julia --project=. src/Jexpresso.jl CompEuler LESICP2-4x4x60-imex
...
function user_inputs()
    ...
    scheme in (:imex, :hevi, :explicit) ||
        error("LESICP2-4x4x60-imex: DBG_SCHEME must be imex, hevi or explicit; got $scheme")
    ...
    restart_dir  = get(ENV, "DBG_RESTART_DIR",
                       "./output/4x4x60-src/CompEuler/LESICP2-4x4x60-imex/output")
    ...
    inputs = Dict(
        ...
        # Interval, not a fixed 10 s: at tend = 60 that is 7 dumps, at
        # tend = 10800 it would be 1081 (~5 GB) and the I/O would outweigh the
        # time loop on a laptop. DBG_DIAG_EVERY=600 gives 19.
        :diagnostics_at_times => (0.0:parse(Float64, get(ENV, "DBG_DIAG_EVERY", "10.0")):tend),
        :lsource              => true,
        :sounding_file        => "./data_files/input_sounding_teamx_u10_flat_noheader.dat",
        #---------------------------------------------------------------------------
        # Integration and quadrature properties
        #---------------------------------------------------------------------------
        # DBG_EXACT=1 switches to over-integration (Q = N+1) -- for testing
        # whether a diagnostic is an inexact-quadrature artifact.
        :lexact_integration   => parse(Bool, get(ENV, "DBG_EXACT", "false")),
        ...
        :lwall_model          => true,
        # Which LGL point above the wall MOST samples the wind at; 2 is the
        # first one. DBG_WALLNODE moves it up without an edit.
        :ifirst_wall_node_index => parse(Int, get(ENV, "DBG_WALLNODE", "2")),
        :bdy_fluxes           => true,
        :lvisc                => true,
        :visc_model           => SMAG(),
        :C_s                  => 0.18,
        ...
        #---------------------------------------------------------------------------
        # Same reasoning. DBG_STAT_START defaults to a third of tend so a
        # short run still writes some and a long one skips the spin-up.
        ...
        #   gmsh -3 problems/CompEuler/LESICP2-4x4x60-imex/LESICP_4x4x60_160m.geo \
        #        -o problems/CompEuler/LESICP2-4x4x60-imex/LESICP_4x4x60_640mX640mX5000m.msh
        #---------------------------------------------------------------------------
        ...
        :lread_gmsh       => true,
        :gmsh_filename    => "./problems/CompEuler/LESICP2-4x4x60-imex/LESICP_4x4x60_640mX640mX5000m.msh",
        ...
        # DBG_STRETCH=0 turns the vertical grading off (uniform 83.3 m elements)
        # without editing this file -- for isolating whether a diagnostic depends
        # on the stretch.
        :lstretch => parse(Bool, get(ENV, "DBG_STRETCH", "true")),
        :stretch_factor => 1.15,
        :stretch_type => get(ENV, "DBG_STRETCHTYPE", "two_block uniformish"),
        :first_zelement_size => parse(Float64, get(ENV, "DBG_FIRSTZ", "10.0")),
        ...
        #---------------------------------------------------------------------------
        :outformat           => "vtk",
        # JEXPRESSO_OUTDIR lets two configurations of this deck run at once on
        # the same machine without writing over each other -- e.g. a two_block
        # control and a powerlaw arm side by side.
        :output_dir          => joinpath(get(ENV, "JEXPRESSO_OUTDIR", "./output"),
                                         "LESICP2-4x4x60-imex_" * String(scheme)) * "/",
