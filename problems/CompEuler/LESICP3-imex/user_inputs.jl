#==============================================================================
 LESICP3-imex -- LESICP3 with all three time integrators behind one switch.

 A copy of problems/CompEuler/LESICP3. Physics, closure, wall model, filter,
 ...
 output directory, and the per-scheme option blocks below.
   ...
   for s in explicit hevi imex; do
     DBG_SCHEME=$s mpiexecjl -n 16 julia --project=. \
       -e 'using Jexpresso; Jexpresso.run_case("CompEuler","LESICP3-imex")'
   done
...
function user_inputs()
    ...
    scheme in (:imex, :hevi, :explicit) ||
        error("LESICP3-imex: DBG_SCHEME must be imex, hevi or explicit; got $scheme")
	...
	:z_transition_end => 2200.0,
        ...
        :output_dir          => "/scratch/smarras/hw59/output_new/LESICP3_scaling-8nodes-64x64x36_10kmX10kmX3dot5km_" * String(scheme) * "/",
        #:output_dir          => "./output",
