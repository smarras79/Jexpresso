...
#=============================================================================
 initialize.jl -- copied verbatim from CompEuler/rtb_hevi, except for the
 qoutvars label noted below.

 The bubble is the 2D CompEuler/theta one: centre (0, 2500) m, radius 2000 m,
 amplitude 2 K, linear taper θc(1 - r/r0) -- and r is measured in the x-z
 plane ONLY, which is what makes it a cylinder along y and the whole reason
 this case can check v == 0. Do not add a y term to r without also deleting
 that check from user_inputs.jl's header.
=============================================================================#
function initialize(SD::NSD_3D, PT, mesh::St_mesh, inputs, OUTPUT_DIR::String, TFloat)
    ...
    #---------------------------------------------------------------------------------
    qvars    = ["ρ", "ρu", "ρv", "ρw", "ρθ"]
    # SLOT 6 IS θ', NOT p. user_primitives.jl's user_uout! writes
    # uout[6] = u[5]/u[1] - qe[5]/qe[1], i.e. the potential-temperature
    # perturbation; the perfectGasLaw line that would have made it a pressure
    # is commented out there. rtb_hevi, which this file is copied from, still
    # labels it "p" and so writes a VTK field named p holding θ'. Named for
    # what it holds here, because θ' is the field this case is looked at
    # through and a mislabelled one is worse than a missing one.
    qoutvars = ["ρ", "u", "v", "w", "θ", "θ_pert"]
