#=============================================================================
 LESICP2-64x64x60-u00-imex -- the intercomparison's FREE CONVECTION case.

 Identical to LESICP2-64x64x60-imex in every respect (mesh, stretch, closure,
 filter, dt, solver, statistics window) except the sounding: u00 instead of
 u10, i.e. no mean wind. Section 2.1 of the setup document, "Flat terrain,
 free convection", filename input_sounding_teamx_u00_flat.

 With U = 0 the surface layer is driven by buoyancy alone: u* collapses to
 whatever MOST_U_MIN (0.1 m/s) supports, the Coriolis/geostrophic source in
 user_source.jl has U_geo = 0 and so does nothing, and the wall node's
 momentum has no mean flow to random-walk against. The wall-damping floor
 still applies; there is simply much less for it to hold back.

 netCDF CASE name for tools/les_to_netcdf.py: "flat_u00".
=============================================================================#
#=============================================================================
...
function user_inputs()
	...
	:output_dir          => joinpath(get(ENV, "JEXPRESSO_OUTDIR",
	                                 ...
	                                 "LESICP2_64x64x60_u00_10240mX10240mX5000m_" * String(scheme)) * "/",
