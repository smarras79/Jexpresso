...
#    if (tag == "free_slip")
    ...
    # `coords` is the (x, y) of the boundary node: unpack it before use.
    # These were bare `x`/`y` and raised UndefVarError on the first
    # Dirichlet edge (never hit with the periodic mesh this case ships).
    x = coords[1]
    y = coords[2]
    if (y < 0.01)
