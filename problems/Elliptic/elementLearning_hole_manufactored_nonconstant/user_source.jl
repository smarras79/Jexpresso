# ============================================================================
#  MANUFACTURED-SOLUTION test that ALSO exercises the GEOMETRY-INDUCED â path.
#
#  Hardwired to el_source_mode() = :mms below: solves the standard Laplacian
#  -Δu = f on the hole domain with f = -Δu_ex and Dirichlet data g = u_ex|_{∂Ω},
#  and stores u_ex in qe so the post-solve L2 error (printed by standard_linsolve!
#  or the element-learning inference) verifies the solver against the analytic
#  solution. To experiment with other right-hand sides, change el_source_mode().
#
#  With :lEL_nonconstant => true (user_inputs.jl) the element-learning surrogate
#  consumes the per-element 2×2 SPD reference diffusivity â = (|K|/|ref|)·J_K⁻¹J_K⁻ᵀ
#  built from the real hole-mesh metrics (a = 1; non-constant purely because the
#  element Jacobians vary). The manufactured solution therefore verifies the
#  surrogate on the geometry-induced â feature it is meant to learn.
#
...
el_source_mode() = :mms      # this problem IS the manufactured-solution test
const FCONST = 1.0
