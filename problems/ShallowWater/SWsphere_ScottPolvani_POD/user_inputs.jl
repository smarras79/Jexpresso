...
#
#   * the GRID is cubed_sphere.msh, the 10×10 panel grid the Galewsky case uses
#     (npoin = 15 002), not the 64×64 one — so a run is minutes rather than
#     hours and the snapshot set is 29 MB rather than 1.2 GB;
#   * SP_NROT defaults to 20 rather than 500, for the same reason;
#   * POD decomposes THREE fields, :vorticity (a derivative) and "h" and
#     :velocity (primitive), because comparing their "sub-element content" lines
#     is what tells a decomposition of the flow from a decomposition of the
#     grid-scale noise that the curl amplifies. See problems/ShallowWater/
#     SWsphere_POD, where that comparison is made on a case that does run.
#
# Everything below this line is the original deck's, unchanged except for those
# four things.
#---------------------------------------------------------------------------------
...
function user_inputs()
    ...
    inputs = Dict(
        ...
        #
        # WHAT IT COSTS. One npoin × ncomp × nsnap array of Float64 per field,
        # held for the run. THIS DECK USES THE 10×10 GRID (npoin = 15 002, the
        # one the Galewsky case runs on), so 61 snapshots of the vorticity, h
        # and the velocity pair together are 29 MB — which is why all three are
        # decomposed here where the 64×64 deck decomposes one. On that grid
        # (npoin ≈ 6.1e5) the same three fields would be 1.2 GB;
        ...
        :pod_nlon             => 720,          # ½°, ample for the 10×10 grid
        :pod_nlat             => 360,
        ...
        :pod_write_snapshots  => true,         # the raw snapshots, for an offline re-run
